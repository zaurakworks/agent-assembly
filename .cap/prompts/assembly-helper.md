...
- 分层闭包：真实用户环境只通过已审批、已绑定的 `real-home` 基座进入；项目 prompt、Skill、MCP、Hook、Plugin 必须位于当前项目声明内，并用显式 `add`、`mask`、`replace` 与基座合成。不得继承未绑定的用户目录、模板、其他仓库或 provider ambient 业务能力。
